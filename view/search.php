        ...
        <div class="container-bottom-right">
            <div class="container-bottom-right-content">
                ...
                <?php           
                    ...
                    if($curr_page>1)
                        echo '<a href="?page=category&idTL='.$idTL.' &index='.($curr_page-1).'">&laquo;</a>';
                    else echo '<a href="?page=category&idTL='.$idTL.' &index=1">&laquo;</a>';

                    for($i=1; $i<=$total_pages; $i++){
                        if($curr_page==$i)
                            echo '<a href="?page=category&idTL='.$idTL.' &index='.$i.'" class="active">'.$i.'</a>';
                        else echo '<a href="?page=category&idTL='.$idTL.' &index='.$i.'">'.$i.'</a>';
                    }

                    if($curr_page>1)
                        echo '<a href="?page=category&idTL='.$idTL.' &index='.($curr_page+1).'">&raquo;</a>';
                    else echo '<a href="?page=category&idTL='.$idTL.' &index='.$total_pages.'">&raquo;</a>';
                ?>
                </div>
            </div>
        </div>
